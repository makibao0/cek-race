...
function CardBack() {
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-center bg-gradient-to-b from-slate-800 to-slate-900 rounded-xl border border-white/10 overflow-hidden">
      ...
      {/* Corner decorations */}
      <span className="absolute top-2 left-2 text-xs text-violet-500/40">
        ✦
      </span>
      <span className="absolute top-2 right-2 text-xs text-violet-500/40">
        ✦
      </span>
      <span className="absolute bottom-2 left-2 text-xs text-violet-500/40">
        ✦
      </span>
      <span className="absolute bottom-2 right-2 text-xs text-violet-500/40">
        ✦
      </span>
    </div>
  );
...
function TarotCard({
  ...
  return (
    <div className="flex flex-col items-center gap-2 w-full lg:w-auto">
      {/* Position label */}
      <div
        className={`text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full bg-gradient-to-r ${positionColors[index]} border ${positionBorders[index]} text-slate-200`}
      >
        {drawn.position}
      </div>
          ...
          >
            ...
            >
              ...
              <div
                ...
                style={{
                  background: `linear-gradient(90deg, transparent, ${drawn.card.warna}, transparent)`,
                }}
              />
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-between p-2 pt-3 pb-3">
                ...
                <span className="text-xs font-bold tracking-widest text-slate-400 uppercase">
                  {String(drawn.card.id).padStart(2, "0")}
                </span>
                ...
                <div
                  ...
                  style={{
                    filter: `drop-shadow(0 0 8px ${drawn.card.warna}80)`,
                  }}
                >
                  {drawn.card.emoji}
                </div>
                ...
                <div className="text-center">
                  <p className="text-xs text-slate-400 uppercase tracking-wider">
                    {drawn.isReversed ? "↕ Terbalik" : "↑ Tegak"}
                  </p>
                  <p
              ...
              <div
                ...
                style={{
                  background: `radial-gradient(circle at 50% 40%, ${drawn.card.warna}, transparent 70%)`,
                }}
              />
            </div>
          </div>
    ...
    try {
      ...
      try {
        const file = await fetch(dataUrl)
          .then((r) => r.blob())
          .then((b) => new File([b], "tarot.png", { type: "image/png" }));
        if (navigator.canShare && navigator.canShare({ files: [file] })) {
          await navigator.share({
            title: "Bacaan Tarotku",
            text: caption,
            files: [file],
          });
      ...
      <div ref={resultRef} className="w-full bg-transparent">
        <div className="relative w-full rounded-2xl bg-slate-900 border border-white/10 p-5 shadow-2xl overflow-hidden">
          ...
          <div
            ...
            style={{
              background:
                "radial-gradient(ellipse at 50% 0%, rgba(124,58,237,0.3), transparent 70%)",
            }}
          />
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <div>
                      <p className={`font-black text-md ${positionColors[i]}`}>
                        {drawn.card.nama}
                        <span className="font-normal text-slate-400 ml-1 text-md">
                          — {drawn.position}
                        </span>
                      </p>
